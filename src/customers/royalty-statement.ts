 ...
 *
 * Uses setDataDir() init pattern for logo path resolution (matching
 * pdf-template.ts pattern from phase 50).
 *
...
import type { RoyaltyReport, RoyaltyLineItem, CustomerAccount } from './types.js';
import {
  BRAND_COLOR,
  BRAND_SECONDARY,
  BRAND_LIGHT,
} from '../orders/pdf-template.js';
import { loadLogoRegistry, resolveLogoPath } from '../logo/registry.js';
 ...
 * Uses the logo registry via dataDir. Returns null if customer has no
 * logos or logo file not found.
 */
function resolveCustomerLogoPath(customer: CustomerAccount): string | null {
  ...
  try {
    ...
    for (const key of customer.logoKeys) {
      ...
      if (entry) {
        const absolutePath = resolveLogoPath(entry);
...
/**
 * Get column definitions for the line items table.
 */
 ...
 * @param customer - The customer account (for logo and contact info)
 * @param _dataDir - Unused; logo resolution uses module-level dataDir set via setDataDir()
 */
export async function generateRoyaltyStatement(
  ...
  return new Promise((resolve, reject) => {
    ...
    drawStatementFooter(doc, report);
