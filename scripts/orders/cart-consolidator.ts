 ...
 * for SanMar.com automation. Groups items by style+color+size, sums quantities,
 * and tracks which order numbers contributed to each cart item.
 *
 * Only SanMar-vendor items with a vendorStyle are included. Items from other
 * vendors (e.g., S&S Activewear) or without a style number are skipped with
 * console warnings.
 *
...
/**
 * Load orders eligible for cart fill from the order store.
 *
 ...
 * @returns Orders eligible for SanMar cart fill, sorted by orderNumber ascending
 */
export async function getOrdersForCartFill(
 ...
 *
 * Logic:
 * 1. Filter line items to SanMar-vendor only (vendor === 'sanmar' or undefined).
 *    Skip items where vendor === 'ss'.
 ...
 */
export function consolidateOrders(orders: Order[]): CartFillRequest {
  /** Map of composite key -> CartItem being built */
  const itemMap = new Map<string, CartItem>();

  for (const order of orders) {
    let sanmarItemCount = 0;

    for (const lineItem of order.lineItems) {
      // Skip non-SanMar vendor items
      if (lineItem.vendor === 'ss') {
        continue;
      }

      // Skip items without a vendorStyle — cannot be added to SanMar cart
      if (!lineItem.vendorStyle) {
        console.warn(
          `[cart-consolidator] Order #${order.orderNumber}: Skipping "${lineItem.productName}" — no vendorStyle defined`,
        );
        continue;
      }
      ...
      if (!lineItem.color || !lineItem.size) {
        console.warn(
          `[cart-consolidator] Order #${order.orderNumber}: Skipping "${lineItem.productName}" (${lineItem.vendorStyle}) — missing color or size`,
        );
        continue;
      }
    ...
    if (sanmarItemCount === 0) {
      console.warn(
        `[cart-consolidator] Order #${order.orderNumber}: Zero SanMar line items after filtering`,
      );
    }
  }
  ...
  if (sortedItems.length === 0) {
    console.warn(
      '[cart-consolidator] Consolidated cart is empty — no items to order from SanMar',
    );
  }
