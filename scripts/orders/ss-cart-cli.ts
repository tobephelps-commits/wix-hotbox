/**
 * S&S Activewear Cart Fill CLI
 *
 * Command-line interface for previewing and executing S&S Activewear cart
 * automation. Shows consolidated cart items from pending orders and
 * optionally launches browser automation to fill the S&S cart.
 *
 * Commands:
 ...
 *
 * Usage:
 *   npx tsx scripts/orders/ss-cart-cli.ts
 *   npx tsx scripts/orders/ss-cart-cli.ts preview
 *   npx tsx scripts/orders/ss-cart-cli.ts --fill
 *   npx tsx scripts/orders/ss-cart-cli.ts --fill --no-headless
 *   npx tsx scripts/orders/ss-cart-cli.ts --status new,ordered
 *   npx tsx scripts/orders/ss-cart-cli.ts help
 *
 * Phase 39: S&S Cart Automation (Plan 03)
 */
...
import type { CartFillResult } from './cart-types.js';
import { getSSOrdersForCartFill, consolidateSSOrders } from './ss-cart-consolidator.js';
import { fillSSCartForPendingOrders } from './ss-cart-filler.js';
...
/**
 * Preview consolidated S&S cart without executing.
...
async function cmdPreview(args: string[]): Promise<void> {
  ...
  console.log(`\n${COLORS.bold}S&S Activewear Cart Preview${COLORS.reset}`);
  ...
  if (request.items.length === 0) {
    console.log(`${COLORS.yellow}No S&S items found after consolidation.${COLORS.reset}`);
    console.log('All items may be from other vendors or missing vendor style numbers.\n');
    return;
  }

  // Count skipped items (non-S&S or missing data)
  const ssLineItems = orders.reduce((sum, o) => {
    return sum + o.lineItems.filter((li) => {
      const isSS = li.vendor === 'ss';
      const hasStyle = !!li.vendorStyle;
      const hasColorSize = !!li.color && !!li.size;
      return isSS && hasStyle && hasColorSize;
    }).length;
  }, 0);
  const skippedCount = totalLineItems - ssLineItems;
  ...
  if (skippedCount > 0) {
    console.log(`${COLORS.yellow}Skipped: ${skippedCount} items (no vendor style, missing color/size, or non-S&S vendor)${COLORS.reset}`);
    console.log('');
  }
...
/**
 * Execute S&S cart fill automation.
 * Calls fillSSCartForPendingOrders and displays results.
 */
async function cmdFill(args: string[]): Promise<void> {
  const statuses = parseStatuses(args);
  const headless = !hasFlag(args, '--no-headless');

  console.log(`\n${COLORS.bold}S&S Activewear Cart Fill${COLORS.reset}`);
  console.log('='.repeat(50));
  ...
  console.log('Starting S&S cart fill automation...\n');

  const result = await fillSSCartForPendingOrders({
    statuses,
    headless,
  });
...
function cmdHelp(): void {
  console.log(`
${COLORS.bold}S&S Activewear Cart Fill CLI${COLORS.reset}

${COLORS.bold}Commands:${COLORS.reset}

  ${COLORS.cyan}preview${COLORS.reset} (default)             Preview consolidated S&S cart without executing
  ${COLORS.cyan}--fill${COLORS.reset}                        Execute S&S cart fill automation
...
${COLORS.bold}Examples:${COLORS.reset}
  ...
  npm run ss-cart                 Preview what would be added to S&S cart
  npm run ss-cart:preview         Same as above
  npm run ss-cart:fill            Execute S&S cart fill automation
  npm run ss-cart:fill -- --no-headless     Run with visible browser
  npm run ss-cart -- --status new,ordered   Preview with multiple statuses
`);
}
